...
> [!IMPORTANT]
> 
> The integer generated by [pcg32's next](#next) and [pcg64's next](#next-1) might be truncated due the type `lua_Integer` used to represent Lua integers be shorter in size. The only way to get the exact data generated by the library [https://www.pcg-random.org/](https://www.pcg-random.org/) is to call [pcg32's nextbytes](#nextbytes) and  [pcg64's nextbytes](#nextbytes-1) and dealing with bytes, because bytes are not truncated.
